import{W as w,M as z,c as p,a as B,f as P,b as m,i as C,o as k}from"./fullscreen.vertex.DM5TiE_a.js";const _=`// Navier-Stokes vorticity-stream function formulation.
...
`,D=8,E=2,S=1,g=E*D-2*S;class L extends w{stepsPerFrame;brushSize;bgColor;mouse;computePL;buf;paramsBuf;computeBG;readbackBuf;nanCheckPending=!1;paramsData=new Float32Array(12);constructor(e){super({canvas:e.canvas,cellSize:e.cellSize??4,updateInterval:e.updateInterval??16}),this.stepsPerFrame=e.stepsPerFrame??1,this.brushSize=e.brushSize??1e3,this.bgColor=e.bgColor??[1,1,1,1]}updateBackground(e){this.bgColor=e,this.paramsData[8]=e[0],this.paramsData[9]=e[1],this.paramsData[10]=e[2],this.paramsData[11]=e[3]}onStart(){this.mouse=new z(this.canvas)}onStop(){this.mouse?.destroy()}buildPipelines(){const e={fullscreen_vertex:B};this.computePL=this.device.createComputePipeline({layout:"auto",compute:{module:p(this.device,_),entryPoint:"main"}});const n=p(this.device,N,e);this.renderPL=this.device.createRenderPipeline({layout:"auto",vertex:{module:n,entryPoint:"vert"},fragment:{module:n,entryPoint:"frag",targets:[{format:this.format}]},primitive:{topology:"triangle-list"}})}buildResources(){const e=this.device,a=this.gw*this.gh*16;this.buf=e.createBuffer({size:a,usage:GPUBufferUsage.STORAGE|GPUBufferUsage.COPY_DST|GPUBufferUsage.COPY_SRC}),this.resetState(),this.paramsBuf=e.createBuffer({size:48,usage:GPUBufferUsage.UNIFORM|GPUBufferUsage.COPY_DST}),this.paramsData.fill(0),this.paramsData[2]=NaN,this.paramsData[4]=this.gw,this.paramsData[5]=this.gh,this.paramsData[8]=this.bgColor[0],this.paramsData[9]=this.bgColor[1],this.paramsData[10]=this.bgColor[2],this.paramsData[11]=this.bgColor[3],e.queue.writeBuffer(this.paramsBuf,0,this.paramsData),this.readbackBuf=e.createBuffer({size:64,usage:GPUBufferUsage.MAP_READ|GPUBufferUsage.COPY_DST}),this.computeBG=e.createBindGroup({layout:this.computePL.getBindGroupLayout(0),entries:[{binding:0,resource:{buffer:this.buf}},{binding:1,resource:{buffer:this.paramsBuf}}]}),this.renderBG=e.createBindGroup({layout:this.renderPL.getBindGroupLayout(0),entries:[{binding:0,resource:{buffer:this.buf}},{binding:1,resource:{buffer:this.paramsBuf}}]})}destroyResources(){this.buf.destroy(),this.paramsBuf.destroy(),this.readbackBuf.destroy(),this.nanCheckPending=!1}resetState(){const e=this.gw*this.gh,n=new Float32Array(e*4),a=Math.min(this.gw,this.gh),r=[{x:.22,y:.28,a:.8,s:.05},{x:.28,y:.33,a:-.8,s:.05},{x:.55,y:.4,a:.4,s:.1},{x:.5,y:.55,a:-.4,s:.1},{x:.72,y:.68,a:1,s:.03},{x:.76,y:.72,a:-1,s:.03},{x:.82,y:.25,a:.6,s:.07},{x:.18,y:.72,a:-.5,s:.04},{x:.3,y:.65,a:.5,s:.04}];for(let t=0;t<this.gh;t++)for(let c=0;c<this.gw;c++){let o=0;for(const l of r){const v=c-l.x*this.gw,d=t-l.y*this.gh,x=v*v+d*d,h=l.s*a;o+=l.a*Math.exp(-x/(2*h*h))}const f=(t*this.gw+c)*4;n[f]=Math.abs(o)/(1+o),n[f+3]=o}this.device.queue.writeBuffer(this.buf,0,n)}frame(){const e=this.mouse.state;if(this.paramsData[0]=e.x*this.gw,this.paramsData[1]=e.y*this.gh,e.active){const a=this.brushSize/this.cellSize;this.paramsData[2]=e.button===2?-a:a}else this.paramsData[2]=NaN;this.device.queue.writeBuffer(this.paramsBuf,0,this.paramsData);const n=this.device.createCommandEncoder();for(let a=0;a<this.stepsPerFrame;a++){const r=n.beginComputePass();r.setPipeline(this.computePL),r.setBindGroup(0,this.computeBG),r.dispatchWorkgroups(Math.ceil(this.gw/g),Math.ceil(this.gh/g)),r.end()}if(P(n,this.ctx.getCurrentTexture().createView(),this.renderPL,this.renderBG),this.device.queue.submit([n.finish()]),!this.nanCheckPending){this.nanCheckPending=!0;const a=[0,(this.gw-1)*16,(this.gh-1)*this.gw*16,((this.gh-1)*this.gw+this.gw-1)*16],r=this.device.createCommandEncoder();for(let t=0;t<4;t++)r.copyBufferToBuffer(this.buf,a[t],this.readbackBuf,t*16,16);this.device.queue.submit([r.finish()]),this.readbackBuf.mapAsync(GPUMapMode.READ).then(()=>{const t=new Float32Array(this.readbackBuf.getMappedRange()),c=isNaN(t[3])&&isNaN(t[7])&&isNaN(t[11])&&isNaN(t[15]);this.readbackBuf.unmap(),this.nanCheckPending=!1,c&&(console.warn("[NavierStokes] NaN in all corners, resetting simulation"),this.resetState())}).catch(()=>{this.nanCheckPending=!1})}}}const b=[1,1,1,1],y=[0,0,0,1],i=document.getElementById("fluid-sim"),G=document.getElementById("fluid-sim-fallback"),u=document.getElementById("fluid-sim-hint");if(i&&u){const s=()=>{u.style.opacity="0",setTimeout(()=>u.remove(),400),i.removeEventListener("pointerdown",s)};i.addEventListener("pointerdown",s)}if(i){const s=new L({canvas:i,cellSize:1,updateInterval:16,stepsPerFrame:1,brushSize:C()?15:30,bgColor:m()?y:b});s.start().then(e=>{e?(G?.remove(),new ResizeObserver(()=>s.handleResize()).observe(i),k(()=>s.updateBackground(m()?y:b))):(i.remove(),u?.remove())})}
